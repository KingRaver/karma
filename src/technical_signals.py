...
# Use global thread manager instead of direct NUMBA imports
...
class UltimateM4TechnicalIndicatorsEngine:
    ...
    def calculate_rsi(self, prices: List[float], period: int = 14) -> float:
        """Calculate RSI using ultra_calc from technical_calculations module"""
        try:
            from technical_calculations import ultra_calc
            return ultra_calc.calculate_rsi(prices, period)
        except (ImportError, AttributeError) as e:
            logger.error(f"Failed to import ultra_calc for RSI calculation: {e}")
    ...
    def calculate_macd(self, prices: List[float], fast: int = 12, 
        ...
        """Calculate MACD using ultra_calc from technical_calculations module"""
        try:
            from technical_calculations import ultra_calc
            return ultra_calc.calculate_macd(prices, fast, slow, signal)
        except (ImportError, AttributeError) as e:
            logger.error(f"Failed to import ultra_calc for MACD calculation: {e}")
            ...
            if len(fast_ema) >= signal:
                macd_history = [fast_ema[i] - slow_ema[i] for i in range(-signal, 0)]
    ...
    def calculate_bollinger_bands(self, prices: List[float], period: int = 20, 
        ...
        """Calculate Bollinger Bands using ultra_calc from technical_calculations module"""
        try:
            from technical_calculations import ultra_calc
            return ultra_calc.calculate_bollinger_bands(prices, period, num_std)
        except (ImportError, AttributeError) as e:
            logger.error(f"Failed to import ultra_calc for Bollinger Bands calculation: {e}")
