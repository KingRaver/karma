...
class ContentAnalyzer:
    ...
    def __init__(self, config=None, db=None, llm_provider=None):
...
"""
    ...
    def filter_already_replied_posts(self, posts: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for post in posts:
            ...
            try:
                # Use the database method (we know it exists from database.py)
                if self.db:
                    already_replied = self.db.check_if_post_replied(post_id, post_url)
                else:
                    logger.logger.warning("No database connection available for reply checking")
                    already_replied = False
            except Exception as e:
                logger.logger.warning(f"Error checking if post was replied to: {str(e)}")
                # If database call fails, assume not replied to avoid duplicates
                already_replied = False
    ...
    def _extract_crypto_terms(self, text: str) -> Dict[str, List[str]]:
    ...
    def generate_response_with_llm(self, post: Dict[str, Any], market_data: Dict[str, Any], llm_provider=None) -> str:
        """
